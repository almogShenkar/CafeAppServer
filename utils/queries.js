...
queries.TodayBestItems= (queryParams)=>{
    return "SELECT ITEM.itemid , ITEM.name , SUM(ORDEREDITEM.qty) total, ITEM.price, ITEM.price * SUM(ORDEREDITEM.qty) TotalSum  FROM ITEM LEFT OUTER JOIN ORDEREDITEM ON ITEM.itemid=ORDEREDITEM.itemid LEFT OUTER JOIN ORDERLIST ON ORDERLIST.olid=ORDEREDITEM.olid WHERE DAY(CURDATE()) = DAY(ORDERLIST.OL_DTTM) GROUP BY ITEM.itemid ORDER BY ORDEREDITEM.qty DESC";
};
...
queries.orderlistreportFromDateToDate= (queryParams)=>{
    ...
    return "SELECT CONCAT(CONCAT(user.firstname,' '),user.lastname) 'fullname',orderlist.userid,olid,ol_dttm,ol_dttm_real,totalprice FROM ORDERLIST JOIN user ON user.userid=ORDERLIST.userid WHERE ol_dttm BETWEEN DATE(\'"+fromDate+"\') AND DATE(\'"+toDate+"\')  ORDER BY ol_dttm DESC" ;
};
